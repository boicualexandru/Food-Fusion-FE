...
export class HotelUserReservationsPageComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.handler = StripeCheckout.configure({
            ...
            source: async (source) => {
                this.hotelService.markReservationAsPaid(this.payingReservationId)
    ...
    payNow(reservation: HotelReservationDetailed) {
        this.payingReservationId = reservation.id;

        const diffTime = Math.abs(new Date(reservation.range.start).getTime() - new Date(reservation.range.end).getTime());
        const daysCount = Math.ceil(diffTime / (1000 * 3600 * 24));

        const totalPayment = reservation.room.price * daysCount;
        ...
        this.handler.open({
            ...
            description: formatDate(reservation.range.start, 'd MMMM, y', 'en') + ' - ' +
                formatDate(reservation.range.end, 'd MMMM, y', 'en'),
